...
<div class="col-sm-12">
  <section class="panel">
	  ...
	  <table class="table table-striped table-advance table-hover">
		  ...
		  <tbody>
		  <?php 
			$sel = $act->selectSort("bahaya", "`id_tempat`", "DESC") ;
			if($sel->rowCount() > 0){
				while($row = $sel->fetchObject()){
			?>
				<tr>
					<td><?php echo $row->nama ;?></td>
					<td><?php echo $row->kriminalitas ;?></td>
					<td><?php echo $row->latitude.",".$row->longitude ;?></td>
					<td><?php 
						$sel2 = $act->selectWhere("user", "`id_user` = '".$row->kontributor."'") ;
						$row2 = $sel2->fetchObject() ;
						
						if($row2->nama ==  "") echo $row2->username ;
						else echo $row2->nama ;
					?></td>
					<td>
						<a href="<?php echo ADMIN.'index.php?modules=bahaya&&action=delete&&id='.$row->id_tempat ;?>" class="btn btn-danger btn-xs"><i class="icon-trash "></i></a>
					</td>
				</tr>
			<?php
				}
